package io.stan.springwebflux.client;
    ...
    public Mono<User> postUser(User user) {
        return webClient
             ...
             //   .doFinally(signalType -> log.info("Result Completed for POST User: {}", signalType));
                .doOnError(throwable -> System.out.println("Result error out for POST user"))
                .doFinally(signalType -> System.out.println("Result Completed for POST User: {}"));
    }
}
